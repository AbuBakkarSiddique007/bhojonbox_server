...
const createOrder = async (userId, data) => {
    ...
    if (meals.length !== data.items.length) {
        throw { status: 400, message: "Some meals are invalid or unavailable" };
    }
...
const updateOrderStatus = async (userId, orderId, status) => {
    ...
    if (!order || order.providerId !== profile.id) {
        throw { status: 404, message: "Order not found or unauthorized" };
    }
    ...
    if (!allowed.includes(status)) {
        throw { status: 400, message: `Cannot change status from ${order.status} to ${status}` };
    }
...
const cancelOrder = async (userId, orderId) => {
    ...
    if (order.status !== "PLACED") {
        throw { status: 400, message: "Can only cancel orders that are still PLACED" };
    }
