...
const toggleUserStatus = async (userId) => {
    ...
    if (user.role === "ADMIN") {
        throw { status: 400, message: "Cannot suspend an admin" };
    }
...
const changeUserRole = async (userId, role) => {
    ...
    if (user.role === "ADMIN") {
        throw { status: 400, message: "Cannot change admin role" };
    }
    const updated = await prisma.user.update({
        where: { id: userId },
        data: { role: role },
...
const getDashboardStats = async () => {
    const [totalUsers, totalCustomers, totalProviders, totalMeals, totalOrders, totalRevenue, totalReviews, ordersByStatus, recentOrders,] = await Promise.all([
        prisma.user.count(),
